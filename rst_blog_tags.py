...
#TAGLINK = '../../../tags/%s.html'
#taglink is an issue if the *.rst files in my research notebook are not all at the same directory level
#one fairly easy solution would be to hard code the absolute path, but that is an issue if I want to publish
#the notebook for my students on my personal website or something
#
#I could hardcode something like #!#TAGROOT#!# that I would later search and replace on all html files,
#but that seems hackish.
#
#The only real issue is that a tag link within a subdirectory document doesn't work.  I could create
#redirection links for those tags.

#Assume most rst entries are in year/semester/project folders
TAGLINK = '../../../tags/%s.html'
...
#create dummy roles and directives for sphinx compatibility
...
def get_title(pathin):
    f = open(pathin, 'r')
    lines = f.read()
    f.close()

    #probably need to call setup_for_plain_docutils here
    document = core.publish_doctree(lines)

    if document.hasattr('title'):
        return document.get('title').encode()
    else:
        return None
